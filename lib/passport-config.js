...
module.exports = function(passport) {
  ...
  passport.use('local-signin', new LocalStrategy({
    ...
    try {
      ...
      // kakao facebook google 인지 확인
      if (user.kakao.id || user.google.id || user.facebook.id){
        return done(null, false ,req.flash('danger', 'kakao , facebook , google 사용자입니다. 다시 로그인 해주세요.'))
      }
      ...
      if (user && await user.validatePassword(password)) {
        ...
        return done(null, false, req.flash('danger', '이메일이나 비밀번호가 틀렸습니다 다시 로그인 해주세요~!'));
      }
    } catch(err) {
      done(err);
    }
